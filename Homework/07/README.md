...
做完這十題後，回頭看 `05/blog_final/app.js`：

- **第 10 題** → 對應所有 `(err, row) => { ... }` 的 callback 結構
- **第 2 題** → 對應 `app.post('/new')` 內取出表單資料
- **第 3 題** → 對應首頁 `posts.forEach` 產生文章列表
- **第 8 題** → 對應 EJS 模板中 `<%= ... %>` 的動態輸出
- **第 5、7 題** → 對應 `db.get('SELECT ...', [id], (err, row) => {...})` 的資料庫查詢模式
